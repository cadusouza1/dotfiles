...
edit :: String -> X ()
edit filePath = spawn $ myTerminal ++ " -e " ++ myEditor ++ " " ++ filePath

fuzzyEdit :: String -> X ()
fuzzyEdit path = spawn $ myTerminal ++ " -e ~/.scripts/fzf/fuzzy-edit.sh " ++ path
...
openBrowserTab url = do
    ...
    spawn $ myBrowser ++ " --new-tab https://" ++ url

notifyLowerVolume :: Double -> X ()
notifyLowerVolume d = do
    v <- lowerVolume d 
    notify 1 $ show v

notifyRaiseVolume :: Double -> X ()
notifyRaiseVolume d = do
    v <- raiseVolume d 
    notify 1 $ show v
...
myKeys = [
    ...
    -- EasyMotion
    , ("M-f", selectWindow def {sKeys = AnyKeys [xK_f, xK_s, xK_j, xK_k]} >>= (`whenJust` windows . W.focusWindow)) 
    ...
    -- Aplications/sites to open
    , ("M-o b", notifyAndSpawn myBrowser)
    , ("M-o t", notifyAndSpawn "teams") 
    , ("M-o d", notifyAndSpawn "discord")
    , ("M-o m", notifyAndSpawn "spotify") 
    , ("M-o g", openBrowserTab "mail.google.com")
    , ("M-o w", openBrowserTab "web.whatsapp.com")
    , ("M-o y", openBrowserTab "youtube.com")
    , ("M-o r", openBrowserTab "reddit.com")
    , ("M-o h", spawnTUI "htop")
    ...
    , ("M-e w", edit "~/.dotfiles/.xmonad/xmonad.hs")
    ...
    -- XMonad Spotify integration
    , ("M-s j", audioPrev)
    , ("M-s k", audioNext)
    , ("M-s p", audioPlayPause)

    , ("M-<F1>", notifyLowerVolume 5)
    , ("M-<F2>", notifyRaiseVolume 5)
